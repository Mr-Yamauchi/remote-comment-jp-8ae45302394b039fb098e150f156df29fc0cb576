...
lrmd_remote_listen(gpointer data)
{
    ...
    static struct mainloop_fd_callbacks lrmd_remote_fd_cb = {
        .dispatch = lrmd_remote_client_msg,						/* 接続クライアントメッセージ処理 */
        .destroy = lrmd_remote_client_destroy,					/* 接続クライアント切断処理 */
    };
...
lrmd_init_remote_tls_server(int port)
{
    ...
    static struct mainloop_fd_callbacks remote_listen_fd_callbacks = {
        .dispatch = lrmd_remote_listen,							/* 接続クライアントからのlisten処理 */
        .destroy = lrmd_remote_connection_destroy,				/* 接続クライアントの切断処理 */
    };
